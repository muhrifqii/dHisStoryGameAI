...
export function Button({ className, variant, size, ...props }) {
  ...
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 aria-invalid:border-destructive ${clsVariant} ${clsSize} ${className}`}
      {...props}
    />
  )
}
    ...
    <div className='w-full' {...props}>
      <div className='flex flex-col md:max-w-4xl mx-auto group'>
        <div className={`flex gap-2 max-w-[60%] relative items-end ${cls}`}>
          {/* <div className={`flex relative items-end ${cls}`}> */}
